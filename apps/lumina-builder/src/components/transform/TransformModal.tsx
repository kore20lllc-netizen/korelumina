...
import { writeRuntimeFile } from "@/services/runtimeService";


function buildTransformFiles(
  mode: TransformMode,
...
async function writeTransformFiles(
  projectId: string,
  mode: TransformMode,
) {
  const files = buildTransformFiles(mode);
...
export function TransformModal() {
  ...
  const completeTransform = useCallback(() => {
    ...
    if (targetId) {
      void writeTransformFiles(targetId, mode).catch((error) => {
        console.error("[TransformModal] Failed to persist transform files", error);
        toast.error("Transform files could not be written to runtime.");
      });
    }
