import {
  ...
  Layers, FileCode2, Wand2, Image as ImageIcon, MessageSquare, Plus, Box,
} from "lucide-react";
...
export function Sidebar() {
  const { mode, view, setView, sidebarOpen, setSidebarOpen, activeProject } = useWorkspace();
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="group relative h-10 w-10 rounded-xl bg-button-lumina grid place-items-center shadow-[0_4px_16px_-4px_hsl(var(--violet)/0.55),inset_0_1px_0_hsl(220_20%_100%/0.18)] hover:brightness-110 transition-all mb-1"
        ...
        >
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                {(activeLabel ? activeLabel === item.label : item.active) && (
                  <span className="absolute -left-2 top-1/2 -translate-y-1/2 h-5 w-0.5 rounded-full bg-violet" />
                )}
                <Icon className="h-[17px] w-[17px]" strokeWidth={1.75} />
                {item.badge && (
                  <span className="absolute -top-0.5 -right-0.5 h-3.5 min-w-3.5 px-1 rounded-full text-[9px] font-semibold grid place-items-center bg-violet text-white">
                    {item.badge}
                  </span>
                )}
