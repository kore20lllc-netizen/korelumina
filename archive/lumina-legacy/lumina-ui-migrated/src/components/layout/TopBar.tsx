import { Sparkles, Bell, Search, Menu, ChevronDown, Terminal, Command, Droplet } from "lucide-react";
    ...
    <header className="relative z-30 h-14 flex items-center gap-3 px-4 md:px-6 glass border-b border-border">
      ...
      <button onClick={() => setView("dashboard")} className="flex items-center gap-2.5 group">
        ...
        <div className="hidden sm:block leading-none">
          ...
          <div className="text-[9px] text-muted-foreground/70 mt-0.5 tracking-[0.18em] uppercase">Studio</div>
        </div>
      </button>
      ...
      <div className="hidden md:flex items-center p-0.5 rounded-xl bg-surface-1 border border-border">
        {modes.map((m) => (
          ...
          >
            {mode === m.id && (
              <span className="absolute inset-0 rounded-lg bg-surface-3 ring-1 ring-white/10" />
            )}
      ...
      <button className="relative h-8 w-8 grid place-items-center rounded-lg hover:bg-surface-2 transition" aria-label="Notifications">
        <Bell className="h-3.5 w-3.5 text-muted-foreground" />
        <span className="absolute top-1.5 right-2 h-1.5 w-1.5 rounded-full bg-violet" />
      </button>

      <LuminaButton size="sm" variant="primary" className="hidden sm:inline-flex" onClick={() => setPublishOpen(true)}>
