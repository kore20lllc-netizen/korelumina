import { startRuntime, deleteProject } from "@/services/api";
import { useEffect, useMemo, useState } from "react";
import { FolderGit2, Plus, Upload, FolderGit2, MoreHorizontal, ArrowLeft, Search, ExternalLink, Pencil, Copy, Trash2, Archive, FolderOpen, Clock, Activity, GitBranch, Globe, BookOpen, X, Link2, RotateCcw, SlidersHorizontal, ArrowDownAZ, ArrowUpAZ, ArrowDownWideNarrow, ArrowUpNarrowWide } from "lucide-react";
...
const REMOVED_IMPORTS_KEY = "lumina_removed_imports";
...
const sourceFor = (p: Project, override?: SourceOverride): ImportSource => {
  ...
  const icon = override?.sourceUrl
    ? FolderGit2
    : kind === "github" ? FolderGit2 : kind === "zip" ? Archive : FolderOpen;
  const label = override?.sourceUrl
    ? "FolderGit2"
    : kind === "github" ? "FolderGit2" : kind === "zip" ? "ZIP archive" : "Local folder";
...
export function ImportsView() {
  ...
  try {
    const raw = localStorage.getItem(REMOVED_IMPORTS_KEY);
    if (!raw) {
      return new Set();
    }
...
useEffect(() => {
  try {
    localStorage.setItem(
      REMOVED_IMPORTS_KEY,
      JSON.stringify(Array.from(removed)),
    );
  } catch {
  ...
  try {
    await deleteProject(removeTarget.projectId || removeTarget.id);
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-7xl mx-auto px-4 md:px-10 py-10 md:py-14">
        <button
          ...
          className="inline-flex items-center gap-1.5 text-[12px] text-muted-foreground hover:text-foreground transition mb-6"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12 anim-in">
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-[13px]">
              {allImports.length} imported · from FolderGit2, ZIP, or a folder
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 px-3 h-9 rounded-lg bg-surface-1 border border-border w-full md:w-64 focus-within:border-white/15 transition">
            ...
            <FilterSelect value={sourceFilter} onChange={(v) => setSourceFilter(v as typeof sourceFilter)} ariaLabel="Filter by source">
              ...
              <option value="github">FolderGit2</option>
        ...
        {allImports.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 bg-surface-1/40 p-12 text-center">
            ...
            <div className="font-display font-semibold text-[16px]">No imported projects yet</div>
            <p className="text-[13px] text-muted-foreground mt-1.5 max-w-sm mx-auto">
              Bring in code from FolderGit2, upload a ZIP archive, or pick a local folder.
            </p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            ...
            >
              ...
              <div className="relative text-center">
                ...
                <div className="font-display font-semibold text-[15px]">Import another</div>
                <div className="text-[11px] text-muted-foreground mt-1">FolderGit2, ZIP, or folder</div>
              </div>
            </button>
          ...
          return (
            ...
            >
              ...
              <div className="px-5 py-4 border-b border-border/60">
                <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground/70">Project</div>
              ...
              <div className="p-5 space-y-3 text-[13px] flex-1 overflow-y-auto">
                ...
                <div className="pt-3 mt-3 border-t border-border/60">
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground/70">Quick links</div>
                    <button
                  ...
                  <div className="grid grid-cols-1 gap-1.5">
                    <QuickLink
                      icon={SrcIcon}
                      label={src.kind === "github" ? "View on FolderGit2" : src.kind === "zip" ? "Download archive" : "Reveal in folder"}
                      hint={src.detail}
      ...
      <Dialog open={!!editTarget} onOpenChange={(o) => { if (!o) setEditTarget(null); }}>
        <DialogContent className="glass-strong border-border max-w-md">
          ...
          {editTarget && (() => {
            ...
            const copyAll = async () => {
              const text = [
                `FolderGit2:  ${effective.source}`,
                `Preview: ${effective.preview}`,
                `README:  ${effective.readme}`,
              ].join("\n");
              try {
                await navigator.clipboard.writeText(text);
                toast.success("All links copied", { description: "FolderGit2, preview, and README" });
              } catch {
    ...
    <select
      ...
      className={cn(
        ...
        isActive
          ...
          : "bg-surface-1 border-border hover:border-white/15"
      )}
    >
      {children}
    </select>
