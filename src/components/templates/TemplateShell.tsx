...
import { auth, usage } from "@/providers/registry";
...
const CAT_TO_INTENT: Record<string, "website" | "webapp" | "dashboard" | "ai-tool" | "mobile"> = {
  Marketing: "website", Website: "website", "Web App": "webapp",
  Dashboard: "dashboard", "AI Tool": "ai-tool", "Mobile App": "mobile",
};
const ACCENT: Array<"violet" | "magenta" | "cyan" | "gold"> = ["magenta", "cyan", "violet", "gold"];
...
export function TemplateShell({ template, children }: TemplateShellProps) {
  ...
  const handleUse = () => {
    ...
    const gate = checkEntitlement("project.create");
    if (!gate.allowed) {
      toast.error(gate.reason ?? "Upgrade required");
      try { window.localStorage.setItem("korelumina:view", "pricing"); } catch {}
      navigate("/");
      return;
    }
